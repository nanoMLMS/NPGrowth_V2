...
FixDeposit::FixDeposit(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg), idiregion(nullptr), idrigid(nullptr),
...
void FixDeposit::init()
{
  ...
  iregion = domain->get_region_by_id(idiregion);
  if (!iregion) error->all(FLERR,"Region ID {} for fix deposit does not exist", idiregion);
...
void FixDeposit::pre_exchange()
{
  ...
  double coord[3], tcoord[3], lamda[3],delx,dely,delz,rsq;
  ...
  while (attempt < maxattempt) {
    ...
    if (targetflag) {
      txlo = tregion->extent_xlo;
      txhi = tregion->extent_xhi;
      tylo = tregion->extent_ylo;
      tyhi = tregion->extent_yhi;
      tzlo = tregion->extent_zlo;
      tzhi = tregion->extent_zhi;
      do {
        tcoord[0] = txlo + random->uniform() * (txhi-txlo);
        tcoord[1] = tylo + random->uniform() * (tyhi-tylo);
        tcoord[2] = tzlo + random->uniform() * (tzhi-tzlo);
      } while (tregion->match(tcoord[0],tcoord[1],tcoord[2]) == 0);
      double vel = sqrt(vnew[0]*vnew[0] + vnew[1]*vnew[1] + vnew[2]*vnew[2]);
      delx = tcoord[0] - coord[0];
      dely = tcoord[1] - coord[1];
      delz = tcoord[2] - coord[2];
...
void FixDeposit::options(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) {
      ...
      idiregion = utils::strdup(arg[iarg+1]);
      iarg += 2;
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix deposit command");
      tregion = domain->get_region_by_id(arg[iarg+1]);
      if (!tregion) error->all(FLERR,"Region ID {} for fix deposit target does not exist",arg[iarg+1]);
      idtregion = utils::strdup(arg[iarg+1]);
      targetflag = 1;
      iarg += 2;
    } else error->all(FLERR,"Illegal fix deposit command");
  }
